...
def add_port_rule(port, action='DROP'):
    ...
    try:
        subprocess.run(f'sudo iptables -A INPUT -p tcp --dport {port} -j {action}', check=True)
        subprocess.run(f'sudo iptables -A OUTPUT -p tcp --dport {port} -j {action}', check=True) 
...
def remove_port_rule(port, action='DROP'):
    ...
    try:
        subprocess.run(f'sudo iptables -D INPUT -p tcp --dport {port} -j {action}', check=True)
